...
function gw_html_contents()
{
    ...
    if ($intKeyUnused > 0) {
        $arStatus[] = array($oL->m('1014'), sprintf(
            '<strong class="red">%s</strong>',
            number_format((($intKbWordlist / $arWordlist['Rows']) * $intKeyUnused), 0, '', ' ')
        )
        );
        $arStatus[] = array($oL->m('1015'), sprintf(
            '<strong class="green">%s</strong>',
        ...
        if ($is == '') {
            $arStatus[] = array('&#160;');
            $arStatus[] = array(sprintf('<strong class="xw">%s</strong>', $oL->m('1016')), sprintf(
...
function getUnusedKeywords()
{
    ...
    foreach ($gw_this['ar_dict_list'] as &$v) {
