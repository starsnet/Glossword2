...
if (isset($ar[0]['ch'])) // Root branch ->
{
    ...
    for ($i0 = 1; $i0 <= $tmp['int_parent_total']; $i0++) // Root -> Topic
    {
        ...
        if (isset($ar[$k]['ch'])) {
            ...
            if (is_array($ar[$k]['ch'])) {
                foreach(array_keys($ar[$k]['ch']) as &$k2)
                {
        ...
        $arTreeId[$k] = $k;
        if(is_array($arTreeId)) {
            foreach(array_keys($arTreeId) as &$kn)
            {
...
require_once $this->sys['path_gwlib'] . '/class.cells_tpl.php';
...
if(is_array($arAlltopics)) {
    foreach($arAlltopics as &$tp)
    {
        /* Topic selected */
