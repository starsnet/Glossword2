...
if (!$this->gw_this['vars']['id']) {
    ...
    foreach ($this->gw_this['ar_dict_list'] as &$v) {
...
if ($this->gw_this['vars']['post'] == '') {
    ...
    if (!$isPostError) { // final update
        ...
        $arPre['date_created'] = @mktime(
            $tmp['ar_created_his'][0],
            $tmp['ar_created_his'][1],
            $tmp['ar_created_his'][2],
            $arPost['date_createdM'],
            $arPost['date_createdD'],
            $arPost['date_createdY']
        );
        ...
        if ($arPre['is_parse_url']) {
            $arPre['parameters']['xml'] = preg_replace(
                "/(^|\[|\s)((http|https|news|ftp|aim|callto):\/\/\w+[^\s\[\\]]+)/i",
                function () {
                    gw_regex_url(array('html' => '\\2', 'show' => '\\2', 'st' => '\\1'));
                },
