...
#prn_r( $this->id_user, 'id_user'  );
    ...
    public function user_get($varname = '')
    {
        if ($varname == '') {
            ...
            if (empty($this->ar_user) || is_string($this->ar_user)) {
                return;
            }
    ...
    public function user_unset($varname = '')
    {
        ...
        if ($varname == '') {
            ...
            foreach (array_keys($this->ar_user['user_settings']) as &$k) {
                if ($k == $varname) {
    ...
    public function auth_info($id_user = '', $username = '', $email = '', $password = '')
    {
        ...
        if ($id_user) {
            /* Select by id_user */
            $sql = sprintf(
                'SELECT `id_user` FROM `%s` WHERE `id_user` = "%s"',
                $this->db_table_users,
                gw_text_sql($id_user)
                    );
        } elseif ($username) {
            /* Select by login */
            $sql = sprintf(
                'SELECT `id_user` FROM `%s` WHERE login = "%s"',
                $this->db_table_users,
                gw_text_sql($username)
                    );
        } elseif ($email) {
            /* Select by email */
            $sql = sprintf(
                'SELECT `id_user` FROM `%s` WHERE `user_email` = "%s"',
                $this->db_table_users,
                gw_text_sql($email)
                    );
        } elseif ($password) {
            /* Select by password */
            $sql = sprintf(
                'SELECT `id_user` FROM `%s` WHERE `password` = "%s"',
                $this->db_table_users,
                gw_text_sql($password)
                    );
        }
        if ($sql != '') {
