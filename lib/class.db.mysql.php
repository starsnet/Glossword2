...
if (!function_exists('mysql_list_dbs')) {
    function mysql_list_dbs($link_id = null)
    {
        return mysqli_query($link_id, 'SHOW DATABASES');
    }
}
...
if (!defined('IS_CLASS_DB')) {
    ...
    class gwtkDataBase
    {
        ...
        public $on_error_default = ON_ERROR_MSG;
         ...
         *
         */
        public function connect($db_host = '', $db_user = '', $db_password = '', $db_name = '', $is_pconnect = 0)
        {
            if ($db_host != '') {
                $this->host = $db_host;
            }
            ...
            if (!$this->link_id) {
                ...
                if (!$this->link_id = @$connect_type($this->host, $this->user, $this->password, $this->database)) {
        ...
        ## ------------------------------------------
        ##
        public function halt($msg, $errMode = 1)
        {
            $this->Error = @mysqli_error($this->link_id);
            $this->Errno = @mysqli_errno($this->link_id);
            if ($errMode == ON_ERROR_IGNORE) {
                return;
            }
            $this->haltmsg($msg);
        ...
        public function table_names($tablename = '')
        {
            $this->database = trim(str_replace('`', '', $this->database));
        ...
        public function MaxId($tablename, $field = 'id')
        {
            if ($tablename != '') {
         ...
         * @access  private
         */
        public function _sql_is_cached($q, $cache_prefix = '')
        {
         ...
         * @access  private
         */
        public function _sql_return($q, $cache_prefix = '')
        {
            global $oCh;
            $oCh->setKey($q, $cache_prefix);
