    ...
    case GW_T_TERM:
        {
            ...
            if (!empty($gw_this['vars']['funcnames'][GW_T_TERM])) {
                foreach ($gw_this['vars']['funcnames'][GW_T_TERM] as &$v) {
            ...
            if (trim($listA['term_1']) != '') {
                $gw_this['ar_breadcrumb'][] = $oHtml->a(
                    $sys['page_index'] . '?' . GW_ACTION . '=' . GW_A_LIST .
    ...
    default:
        {
            ...
            $oTpl->addVal(
                ...
                '<span class="gray">' . (date_extract_int($arStatCommon['date'], "%d") / 1) . date_extract_int($arStatCommon['date'], (" %FL %Y")) . '</span>'
                            ...
                            . '<br />' . $oL->m('stat_defn') . ': ' . $oFunc->number_format($arStatCommon['sum'], 0, $oL->languagelist('4')),
                'xt'
            )
            );
            ## ------------------------------------------------
