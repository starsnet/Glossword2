...
//prn_r( $qT );
                    ...
                    /* Select custom rules for uppercasing */
                    foreach ($arCustomAZOrder as &$arV) {
    ...
    if (isset($gw_this['vars']['arPost']['id_session'])) {
        ...
        */
        ...
        foreach ($ar_lines_csv as $k1 => $v1) {
            ...
            /* skip empty terms */
            if (!isset($ar_line_csv[1]) || trim($ar_line_csv[1]) == '') {
                continue;
            }
            /* support for incomplete records */
            if (!isset($ar_line_csv[$arFields[-2][5]])) {
                $ar_line_csv[$arFields[-2][5]] = '';
            }
            if (!isset($ar_line_csv[$arFields[-3][5]])) {
                $ar_line_csv[$arFields[-3][5]] = '';
            }
            if (!isset($ar_line_csv[$arFields[-4][5]])) {
                $ar_line_csv[$arFields[-4][5]] = '';
            }
            if (!isset($ar_line_csv[$arFields[-5][5]])) {
                $ar_line_csv[$arFields[-5][5]] = '';
            }
    ...
    */
    ...
    foreach ($arXML as $uid => $v1) {
        ...
        if (isset($arDuplicates[$uid])) {
            foreach ($arDuplicates[$uid] as &$arVd) {
                unset($arVd[1]);
    ...
    if (is_array($vars)) {
        foreach ($vars as $key => $val) {
            ...
            if (isset($arBroken[$key])) {
                $arBrokenMsg[$key] = '<div class="red"><strong>' . $oL->m('reason_9') .'</strong></div>';
            }
        } // end of while
    }
...
// --------------------------------------------------------
// Prepare variables
if ($gw_this['vars']['arPost'] == '') {
    $gw_this['vars']['arPost'] = array();
}
if (!isset($gw_this['vars']['arPost'][GW_ACTION])) {
    $gw_this['vars']['arPost'][GW_ACTION] = GW_A_ADD;
}
