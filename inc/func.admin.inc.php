...
class gw_topics_recounter
{
    ...
    /* */
    public function get_subitems($id_topic, $ar_items_counted, $ar_parents)
    {
...
function gw_ParsePre($arParsed, $arPre)
{
    ...
    foreach (array_keys($arPre) as &$target_name) {
        // replace structures
        ...
        if (isset($arControl[$target_name])) { // defn | abbr | trns
            ...
            foreach ($arControl[$target_name] as $action => $arId) {
                ...
                foreach ($arId as $elK => $arCh) {
                    foreach (array_keys($arCh) as &$chK) {
                        $tmp['chK'] = $chK;
                        $tmp['elK'] = $elK;
                    }
                }
