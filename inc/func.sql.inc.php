...
function gwAddTerm($arPre, $id_dict, $arStop, $in_term, $is_specialchars, $is_overwrite, $intLine = 0, $isDelete = 0)
{
    ...
    if ($isQ) {
        ...
        foreach (array_keys($arFields) as &$fK) {
            if ($fK != 0) {
        ...
        foreach ($gw_this['vars']['funcnames'][GW_A_UPDATE . GW_T_TERM] as &$v) {
...
function gwAddNewKeywords($id_dict, $id_term, $arKeywords, $termIdOld, $isClean, $date_created)
{
    ...
    if (!empty($arSql)) { // some keywords are exist already
        ...
        foreach ($arSql as $arK => $arV) {
            ...
            // Set Field ID
            foreach (array_keys($arFields) as &$id_field) {
    ...
    foreach (array_keys($arKeywordsJoin) as &$newkeyword) {
        $q2['word_id']++;
        $q2['dict_id'] = $id_dict;
        $q2['term_id'] = $id_term;
        $q2['date_created'] = $date_created;
