...
#				$this->setTag('textarea', 'onkeyup', 'storeCaret(this)');
            ...
            if ($tmp['intFormHeight'] <= 3) {
                $tmp['intFormHeight'] = 4;
            }
            $tmp['strform'] .= sprintf(
                '<tr style="vertical-align:top"><td style="text-align:%s" class="%s">%s:%s</td>',
                $this->Gsys['css_align_right'],
                $this->Gtmp['cssTrClass'],
                $this->oL->m($fieldname),
                ''
                                );
    ...
    public function make_see($fieldname, $ar = array())
    {
        ...
        $tmp['strform'] .= sprintf(
            '<td style="text-align:%s" class="%s">%s:%s</td>',
            $this->Gsys['css_align_right'],
            $this->Gtmp['cssTrClass'],
            $this->oL->m($fieldname),
            ''
