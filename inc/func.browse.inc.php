...
function gw_sql2defnpreview($arSql)
{
    ...
    foreach ($arPreview as $k => $arV) {
        ...
        if (!empty($arDuplicates[$k])) {
            foreach ($arDuplicates[$k] as &$arVd) {
                /* remove some fields from definition preview */
        ...
        if ($int_defn_length > $sys['int_max_char_defn']) {
            ...
            if (GW_IS_BROWSE_ADMIN) {
                ...
                if ($gw_this['vars']['a'] == GW_A_SEARCH) {
                    $arA[$k]['defn'] = str_replace(
                        $sys['txt_magic_splitter'],
                        $oHtml->a($href_term, '&#8230;'),
                        $arA[$k]['defn']
                    );
                } else {
...
function gw_custom_page($id_page)
{
    ...
    foreach (array_keys($ar_parents) as &$page_k) {
        ...
        $ar_page_titles[] = $oHtml->a(
            $sys['page_index'] .
            '?a='.GW_A_CUSTOMPAGE.'&id=' . $str_page_id,
            $arSqlc[$page_k]['title']
        );
    }
    ...
    if (isset($arSqlc[$id_page_int]['ch'])) {
        ...
        foreach (array_keys($ar_subpages) as &$page_k) {
            switch ($sys['pages_link_mode']) {
                case GW_PAGE_LINK_NAME:
            ...
            $arVarPage[$page_k]['url:page_title'] = $oHtml->a(
                $sys['page_index'] .
                '?a='.GW_A_CUSTOMPAGE.'&id=' . $str_page_id,
                $arSqlc[$page_k]['title']
            );
...
function getTop10($m, $amount = 10, $isItemOnly = 0, $order = 0, $top10_display = 1)
{
    ...
    if (!empty($arThText) || !empty($arThWidth)) {
        ...
        if (!empty($arThText)) {
            ...
            $str_head .= '</tr>';
        } elseif (!empty($arThWidth)) {
            $str_foot .= '<tr>';
            $str_foot .= '<th style="font-size:1px;height:1px;width:1%"></th>';
            foreach (array_keys($arThWidth) as &$kT) {
                $str_width = '';
                if (isset($arThWidth[$kT]) && ($arThWidth[$kT] != '')) {
...
function getDictParam($id_dict)
{
    ...
    foreach ($gw_this['ar_dict_list'] as $vDict) {
...
function getTermParam($tid = '', $name = '')
{
    ...
    if ($tid) {
        ...
        // compare founded values (Q) with imported (T)
        foreach ($arSql as &$arV) {
            $isTermExist = 0;
