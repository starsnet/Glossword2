...
class gw_restore_quotes
{
    ...
    /* */
    public function _attr_fix($str_attr, $tag)
    {
        if (trim($tag) == '') {
            return '';
        }
    ...
    /* */
    public function _attr_expand($str_attr)
    {
        $ar_pairs = $ar = array();
        if (trim($str_attr) == '') {
            return $ar;
...
function validatePostWalk($a, $reqFieldsA = array())
{
    $brokenFieldsA = array();
    foreach ($a as $k1 => $v1) {
        foreach ($reqFieldsA as &$reqv1) {
            if ($k1 == $reqv1) { // posted == required
                if (!is_array($v1)) {
                    $v1 = gw_text_sql($v1);
                    // url check
                    if ($reqv1 == 'url') {
                        if (str_replace("http://", "", $v1) == '') {
                            $v1 = '';
                        }
                    }
                    if ($v1 == '') {
...
function htmlFormSelectDate($name, $val)
{
    ...
    if (preg_match("/([0-9]{4})([0-9]{2})([0-9]{2})([0-9]{6})/", $val, $rowDate)) {
        ...
        for ($m = 0; $m <= 12; $m++) {
            ...
            if ($m == 0) {
                if ($cfgIsNulls) {
                    $strA[1] .= '<option value="' . sprintf("%'02s", $m) . '"' . $s . '>' . sprintf("%'02s", $m) . " " . $arLs[$m] . '</option>';
                }
            } else {
        ...
        for ($d = 0; $d <= 31; $d++) {
            ...
            if ($d == 0) {
                if ($cfgIsNulls) {
                    $strA[2] .= '<option value="' . sprintf("%'02s", $d) . '"' . $s . '>' . sprintf("%'02s", $d) . '</option>';
                }
            } else {
                $strA[2] .= '<option value="' . sprintf("%'02s", $d) . '"' . $s . '>' . sprintf("%'02s", $d) . '</option>';
            }
