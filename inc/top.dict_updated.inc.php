    ...
    foreach ($arSql as &$arV) {
        ...
        if (GW_IS_BROWSE_ADMIN) {
            if ($oSess->is('is-sys-settings')
                ...
                $strData .= $oHtml->a($sys['page_admin'].'?'.GW_ACTION. '='.GW_A_EDIT .'&'. GW_TARGET.'='.GW_T_DICTS
                    . '&id='.$arV['id'] . '&tid='.$arV['id'], $oL->m('3_edit'), $oL->m('1335').': '.$oL->m('3_edit'));
                $strData .= ' ';
                $strData .= $oHtml->a($sys['page_admin'].'?'.GW_ACTION. '='. GW_A_ADD .'&'. GW_TARGET.'='.GW_T_TERMS
                    . '&id='.$arV['id'] . '&tid='.$arV['id'], $oL->m('3_add_term'), $oL->m('terms').': '.$oL->m('3_add'));
                $strData .= '&#160;</span></td>';
                $strData .= '<td class="termpreview">' . $oHtml->a($sys['page_admin'].'?'.GW_ACTION. '='.GW_A_EDIT
                    .'&'. GW_TARGET.'='.GW_T_DICTS. '&id='.$arV['id'] . '&tid='.$arV['id'], $arV['title']) . '</td>';
            } else {
