...
function gw_get_stopwords($arDictParam)
{
    ...
    if ($arDictParam['is_filter_stopwords'] && is_array($arDictParam['ar_filter_stopwords'])) {
        foreach (array_keys($arDictParam['ar_filter_stopwords']) as &$locale_id) {
...
#				$sql_table_match = ', `'.$arDictParam['tablename'].'` AS dict';
            ...
            default:
                /* Search by fields */
                foreach ($a_search_params['in'] as &$inV) {
                    $tmp['arIn'][] = $inV;
                }
        ...
        if ($a_search_params['adv'] == 'all'
            ...
            {
                foreach ($arDict_Ids as &$id_dict) {
                    /* Go for each word */
                    foreach ($a_keywords as &$v) {
...
#exit;
        ...
        } elseif ($a_search_params['adv'] == 'exact') {
            /* ----------------------------------------- */
            ...
            foreach ($arDict_Ids as $kk => $id_dict) {
                ...
                foreach ($tmp['a_results_temp'] as $id_dict_results => $a_results) {
                    ...
                    foreach (array_keys($a_results) as &$rK) {
                        /* Collect matched Term IDs */
    ...
    if (GW_IS_BROWSE_WEB
        ...
        foreach ($tmp['arCache']['srch_settings']['results'] as &$id_term) {
            gwtk_header(
                $sys['server_proto'] . $sys['server_host'] .
                    $oHtml->url_normalize(
                        $sys['page_index'] . '?' .
                            GW_ACTION . '=' . GW_T_TERM . '&' .
...
function gw_search_results($id_srch, $p, $id_dict = 0)
{
    ...
    foreach (array_keys($arSql['results']) as &$dictK) {
        $tmp['arDictParam'][$dictK] = getDictParam($dictK);
        if ($cnt_dict == 1) {
    ...
    if (GW_IS_BROWSE_ADMIN) {
        $arResults = $oDb->sqlExec(
            $oSqlQ->getQ(
                            'get-terms-in-adm',
                            $tmp['arDictParam'][$id_dict]['tablename'],
                            $str_terms,
                            $arDictParam['az_sql'],
    ...
    foreach ($arSql['arDictIds'] as &$v_id_dict) {
    ...
    foreach ($arA as $k1 => $v1) {
        ...
        if (GW_IS_BROWSE_ADMIN) {
            ...
            if ($v1['is_active'] == 3) {
                ...
                $oTpl->tmp['d']['search_item'][$k1]['v:term'] = $oHtml->a(
                    $page_index
