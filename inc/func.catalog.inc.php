...
function gw_get_dict_terms($dict_tablename, $id_dict)
{
    ...
    /* For each letter */
    foreach (array_keys($ar0z) as &$letter) {
            ...
            foreach ($ar[$p]['ch'] as $k => $v) {
                ...
                if ($cntTopic > -1) { // unlimit topics
                    ...
                    if (isset($arDictMap[$k])) { // dictionary found
                        $cntDict = 0;
                        $str .= CRLF . '<dl>';
                        if (is_array($arDictMap[$k])) {
                            foreach ($arDictMap[$k] as &$v2) {
                                $strMark = '';
                                $idcolor = '#999';
                                ...
                                if ($cntDict < $dict_nmax) { // dictionaries limit
                                    ...
                                    $str .= '<dt class="xt">' . ($sys['is_list_images'] ? $strIcon : '') . $oHtml->a(
                                        $sys['page_index']  . "?a=index&d=" . $v2['uri'],
                                        $v2['title']
                                    );
...
function gw_rearrange_to_tree($arSql, $id = 0, $id_name = 'id_page')
{
    ...
    foreach ($arSql as &$arV) {
        ...
        $arStr[$p]['ch'][$i] = $i;
        $arStr[$p]['max'] = $i;
        if (!isset($arStr[$p]['max'])) {
            $arStr[$p]['max'] = $i;
        }
        if (!isset($arStr[$p]['min'])) {
            $arStr[$p]['min'] = $i;
        }
    }
    /* Merge */
    if (is_array($arStr2)) {
        foreach ($arStr2 as $key => $val) {
            if (isset($arStr[$key]) && is_array($val)) {
...
{
    global $arId;
    if (isset($ar[$id]['ch']) && is_array($ar[$id]['ch'])) {
        foreach (array_keys($ar[$id]['ch']) as &$k) {
            if (isset($ar[$k]['ch'])) {
                ctlgGetTree($ar, $k);
            }
            $arId[$k] = $k;
    ...
    if (sizeof($ar) > 0) {
        if ($startId != 0) {
            ...
            elseif ($topic_mode == 'html') {
                ...
                } elseif ($gw_this['vars'][GW_TARGET] == 'custom-pages') {
                ...
                if (isset($ar[$startId]['int_items'])) {
                    ...
                    if ($ar[$startId]['int_items'] > 0) {
                        $str .= $oHtml->a(
                            $page_index . '?'.GW_ACTION.'='.GW_A_BROWSE.'&'.GW_TARGET.'='.GW_T_DICTS.'&w1='.$ar[$startId]['id'],
